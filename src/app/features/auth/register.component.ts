    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 p-4">
      <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <div class="text-center mb-8">
          ...
          <p class="text-slate-600">Crea tu cuenta</p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-6">
          ...
          <div class="text-center text-sm text-slate-600">
            ¿Ya tienes cuenta? 
            <a routerLink="/login" class="text-indigo-600 hover:text-indigo-700 font-medium">Inicia sesión</a>
          </div>
        </form>
      </div>
      <p-toast />
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      this.authService.register(email!, password!, name!).subscribe({
        ...
        error: (error) => {
          this.loading = false;
          this.messageService.add({ severity: 'error', summary: 'Error', detail: error.error?.message || 'Error al crear la cuenta' });
        }
      });
    }
  }
